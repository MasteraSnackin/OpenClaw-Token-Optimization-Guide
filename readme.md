...
#Single master prompt for your agent
...
When you need past context, use your memory_search or equivalent targeted lookup instead of reading entire files. Search by relevant keywords, then load only the hits or small excerpts.
...
Budget awareness (add this to your core operating rules)

Assume I care about keeping monthly LLM spend low.

Prefer: local / free models (if configured) → cheap models (Haiku, similar) → mid‑tier (Sonnet) → Opus only when strictly necessary.
...
When you are done applying all feasible changes:
